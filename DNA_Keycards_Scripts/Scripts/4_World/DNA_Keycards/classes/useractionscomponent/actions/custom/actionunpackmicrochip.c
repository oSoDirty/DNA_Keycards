class ActionUnpackMicrochip: ActionUnpackBox
{
    override void OnFinishProgressServer( ActionData action_data )
    {
        if ( action_data.m_MainItem && action_data.m_MainItem.GetHierarchyRootPlayer() == action_data.m_Player )
        {
            ...
            if( GetGame().ConfigIsExisting( path ) && GetGame().ConfigIsExisting( path + " Resources") )
            {
                ...
                ItemBase emptyBlister = ItemBase.Cast( GetGame().CreateObjectEx("DNA_KeyCard_BlisterPackChipEmpty", action_data.m_Player.GetPosition(), ECE_PLACE_ON_SURFACE) );               

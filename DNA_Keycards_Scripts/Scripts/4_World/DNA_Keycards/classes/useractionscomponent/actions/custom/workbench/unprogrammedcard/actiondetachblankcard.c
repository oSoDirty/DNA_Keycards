class ActionDetachBlankCard: ActionInteractBase
{
	void ActionDetachBlankCard()
	{
	...
	override bool ActionCondition( PlayerBase player, ActionTarget target, ItemBase item )
	{
		...
		if(action_selection && action_selection == "reader")
		{
			...
			if(CastTo(workbench, target_object))
				return workbench.IsBlankCardAttached() && !workbench.IsInUse() && !player.GetItemInHands();
		}
		return false;
	}
	...
	override void OnExecuteServer( ActionData action_data )
	{
		...
		if(CastTo(workbench, action_data.m_Target.GetObject()))
			workbench.ReturnBlankCardToPlayer(action_data.m_Player);
	}
	...
	override void OnExecuteClient(ActionData action_data)
	{
		...
		if(CastTo(workbench, target_object))
			DNA_Sounds.SoundKCCardRemovePlay(workbench.GetPosition());
	}
}
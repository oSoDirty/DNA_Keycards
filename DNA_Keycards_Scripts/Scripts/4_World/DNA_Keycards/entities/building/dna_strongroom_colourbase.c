class DNA_Strongroom_ColourBase: BuildingSuper
{
	...
	static ref array<DNA_Strongroom_ColourBase> m_Strongrooms = new array<DNA_Strongroom_ColourBase>;
	int m_TimeOfOpening;
	...
	void ~DNA_Strongroom_ColourBase()
	{
		if(m_Strongrooms)
		{
			int localIndex = m_Strongrooms.Find(this);
			if(localIndex >=0)
				m_Strongrooms.Remove(localIndex);
		}
	}

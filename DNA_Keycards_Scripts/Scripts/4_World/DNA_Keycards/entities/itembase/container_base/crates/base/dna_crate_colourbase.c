class DNA_Crate_ColourBase: Container_Base
{
	...
	static ref array<DNA_Crate_ColourBase> m_Crates = new array<DNA_Crate_ColourBase>;
	int m_TimeOfOpening;
	...
	{
		super.EEInit();
		if(!GetType().Contains("Smol")) 
			m_Crates.Insert(this);
	}
	...
	void ~DNA_Crate_ColourBase()
	{
		if(m_Crates)
		{
			int localIndex = m_Crates.Find(this);
			if(localIndex >=0)
				m_Crates.Remove(localIndex);
		}
	}

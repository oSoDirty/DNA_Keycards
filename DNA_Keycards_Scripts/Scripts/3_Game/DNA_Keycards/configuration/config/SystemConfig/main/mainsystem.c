...
{	
	...
	static void CheckForConfigUpdate() //Replaces v1 with v2
	{
		...
		array<string> configPaths =
		{
			...
			dna_VaultSystemConfigPath,
			dna_BasicConfigPath,
			dna_YellowConfigPath,
			dna_GreenConfigPath,
			dna_BlueConfigPath,
			dna_PurpleConfigPath,
			dna_RedConfigPath
		};
		...
		if(FileExist(dna_ProfilePath + dna_ConfigFolderName) && dna_ConfigcppVersion != DNA_Keycards_Main_System.GetInstance().GetConfig().GetConfigData().GetVersionArray().Get(0).GetConfigVersion())
		{
			...
			for(int i = 0; i < categoties.Count(); i++)
			{
				...
				{
					string backupConfig = dna_BackupConfigFolderPath + "/" + categoties.Get(i) + ".json";
			...
			{
				DeleteFile(dna_ProfilePath + dna_ConfigFolderName + "/" + directories[j]);
			}
			dna_RecentlyUpdated = true;
			for(int k = 0; k < 99; k++)
			{
				Print("[DNA Debug] - YOU ARE USING AN OUTDATED CONFIG!!! Creating backups in Profile/DNA_Keycards/ConfigBackups and placing new defaults. Don't forget to merge your old settings!!!");
			}
		}
		//dna_UpdateCheckComplete = true;
		
		/*SAVE FOR FUTURE USE   v2 to v3  ?!?!*/
		/*
		array<string> configPaths =
		{
			dna_MainSystemConfigPath,

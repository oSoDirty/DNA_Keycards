...
{	
	...
	{		
        ...
        {
			if(!KeyCard_Config_Main_System.RecentlyUpdated()) Print("[KCM_Debug] - DNA_Keycards Red Tier: Config not found, creating default config!");
            CreateDefaultConfig();
        }
		else
		{
	...
	static void CreateDefaultConfig()
	{
		...
		m_KeyCard_Red_ConfigData.GetWeaponVarsArray().Insert(new Weapon_Variables_Red("(6) Min Weapons to give for Red Tier Vault (Can turn off in system config)", 1));
		m_KeyCard_Red_ConfigData.GetWeaponVarsArray().Insert(new Weapon_Variables_Red("(7) Max Weapons to give for Red Tier Vault Vault (Don't set lower than min!)", 3));
		m_KeyCard_Red_ConfigData.GetWeaponVarsArray().Insert(new Weapon_Variables_Red("(8) Min Extra Mags to give for Red Tier Vault Main Weapon (Can turn off in system config)", 2));
		m_KeyCard_Red_ConfigData.GetWeaponVarsArray().Insert(new Weapon_Variables_Red("(9) Max Extra Mags to give for Red Tier Vault Main Weapon (Don't set lower than min!)", 4));
		m_KeyCard_Red_ConfigData.GetWeaponVarsArray().Insert(new Weapon_Variables_Red("(10) Min Extra Ammo to give for Red Tier Vault Main Weapon (Can turn off in system config)", 1));
		m_KeyCard_Red_ConfigData.GetWeaponVarsArray().Insert(new Weapon_Variables_Red("(11) Max Extra Ammo to give for Red Tier Vault Main Weapon (Don't set lower than min!)", 2));

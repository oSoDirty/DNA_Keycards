...
{	
	...
	protected static void LoadConfig()
	{
		...
		if (!FileExist(kcm_BasicConfigPath))
        {
			if(!KeyCard_Config_Main_System.RecentlyUpdated()) Print("[KCM_Debug] - DNA_Keycards Basic Settings: Config not found, creating default config!");
            CreateDefaultConfig();
        }
		else
		{
			JsonFileLoader<KeyCard_ConfigData_Base>.JsonLoadFile(kcm_BasicConfigPath, m_KeyCard_Base_ConfigData);
			Print("[KCM Debug] - DNA_Keycards Basic Settings: Config found, loading settings!");
		}
	}
	...
	static void CreateDefaultConfig()
	{
		...
		JsonFileLoader<KeyCard_ConfigData_Base>.JsonSaveFile(kcm_BasicConfigPath, m_KeyCard_Base_ConfigData);
	}
}
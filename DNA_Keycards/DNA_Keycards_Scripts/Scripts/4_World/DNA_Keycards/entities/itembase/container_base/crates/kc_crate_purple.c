...
{
	protected ref array<ref KCM_Weapons_Purple> kcm_Loadouts;
	protected ref array<ref KCM_Clothing_P> kcm_Outfits;
	protected ref array<ref Weapon_Variables_Purple> kcm_WeaponVars;
	
	void KC_Crate_Purple()
	{
		kcm_Loadouts = DNA_Keycards_Purple.GetInstance().GetConfig().GetConfigData().GetWeaponsArray();
		kcm_Outfits = DNA_Keycards_Purple.GetInstance().GetConfig().GetConfigData().GetClothingArray();
		kcm_WeaponVars = DNA_Keycards_Purple.GetInstance().GetConfig().GetConfigData().GetWeaponVarsArray();
	}
	...
	void GiveMainWeapon()
	{
		...
		if(CanSpawnMainWeapon(color))
		{
			for(i = 0; i < choice; i++)
			{
				KCM_Weapons_Purple mainLoadout = kcm_Loadouts.GetRandomElement();
	...
	void GiveOutfit()
	{
		...
		if(CanSpawnClothes("purple"))
		{
			KCM_Clothing_P clothingSet = kcm_Outfits.GetRandomElement();

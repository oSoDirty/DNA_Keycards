...
{	
	protected ref array<ref KCM_Weapons_Red> kcm_Loadouts;
	protected ref array<ref KCM_Clothing_R> kcm_Outfits;
	protected ref array<ref Weapon_Variables_Red> kcm_WeaponVars;
	
	void KC_Crate_Red()
	{
		kcm_Loadouts = DNA_Keycards_Red.GetInstance().GetConfig().GetConfigData().GetWeaponsArray();
		kcm_Outfits = DNA_Keycards_Red.GetInstance().GetConfig().GetConfigData().GetClothingArray();
		kcm_WeaponVars = DNA_Keycards_Red.GetInstance().GetConfig().GetConfigData().GetWeaponVarsArray();
	}
	...
	void GiveMainWeapon()
	{
		...
		if(CanSpawnMainWeapon(color))
		{
			for(i = 0; i < choice; i++)
			{
				KCM_Weapons_Red mainLoadout = kcm_Loadouts.GetRandomElement();
	...
	void GiveOutfit()
	{
		...
		if(CanSpawnClothes("red"))
		{
			KCM_Clothing_R clothingSet = kcm_Outfits.GetRandomElement();

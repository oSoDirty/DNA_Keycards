...
{
	protected ref array<ref KCM_Weapons_Green> kcm_Loadouts;
	protected ref array<ref KCM_Clothing_G> kcm_Outfits;
	protected ref array<ref Weapon_Variables_Green> kcm_WeaponVars;
	
	void KC_Crate_Green()
	{
		kcm_Loadouts = DNA_Keycards_Green.GetInstance().GetConfig().GetConfigData().GetWeaponsArray();
		kcm_Outfits = DNA_Keycards_Green.GetInstance().GetConfig().GetConfigData().GetClothingArray();
		kcm_WeaponVars = DNA_Keycards_Green.GetInstance().GetConfig().GetConfigData().GetWeaponVarsArray();
	}
	...
	void GiveMainWeapon()
	{
		...
		if(CanSpawnMainWeapon(color))
		{
			for(i = 0; i < choice; i++)
			{
				KCM_Weapons_Green mainLoadout = kcm_Loadouts.GetRandomElement();
	...
	void GiveOutfit()
	{
		...
		if(CanSpawnClothes("green"))
		{
			KCM_Clothing_G clothingSet = kcm_Outfits.GetRandomElement();

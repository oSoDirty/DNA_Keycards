...
{
	protected ref array<ref KCM_Weapons_Blue> kcm_Loadouts;
	protected ref array<ref KCM_Clothing_B> kcm_Outfits;
	protected ref array<ref Weapon_Variables_Blue> kcm_WeaponVars;
	
	void KC_Crate_Blue()
	{
		kcm_Loadouts = DNA_Keycards_Blue.GetInstance().GetConfig().GetConfigData().GetWeaponsArray();
		kcm_Outfits = DNA_Keycards_Blue.GetInstance().GetConfig().GetConfigData().GetClothingArray();
		kcm_WeaponVars = DNA_Keycards_Blue.GetInstance().GetConfig().GetConfigData().GetWeaponVarsArray();
	}
	...
	void GiveMainWeapon()
	{
		...
		if(CanSpawnMainWeapon(color))
		{
			for(i = 0; i < choice; i++)
			{
				KCM_Weapons_Blue mainLoadout = kcm_Loadouts.GetRandomElement();
	...
	void GiveOutfit()
	{
		...
		if(CanSpawnClothes("blue"))
		{
			KCM_Clothing_B clothingSet = kcm_Outfits.GetRandomElement();

...
{	
	...
	{
		if (!FileExist(kcm_ProfilePath + m_ConfigFolderName + "/Yellow/"))
        {
			Print("[KCM_Debug] - KeyCardMod Yellow Tier: Directory not found, creating directory!");
			MakeDirectory(kcm_ProfilePath + m_ConfigFolderName + "/Yellow/");
		}
		
		m_KeyCard_Yellow_ConfigData = new KeyCard_ConfigData_Yellow();
		
		if (!FileExist(kcm_ProfilePath + m_ConfigFolderName + "/Yellow/" + m_ConfigName + "Yellow" + m_ConfigExtension))
        {
			Print("[KCM_Debug] - KeyCardMod Yellow Tier: Config not found, creating default config!");
            CreateDefaultConfig();
        }
		else
		{
			JsonFileLoader<KeyCard_ConfigData_Yellow>.JsonLoadFile(kcm_ProfilePath + m_ConfigFolderName + "/Yellow/" + m_ConfigName + "Yellow" + m_ConfigExtension, m_KeyCard_Yellow_ConfigData);
		}
	}
	...
	static void CreateDefaultConfig()
	{
		...
		JsonFileLoader<KeyCard_ConfigData_Yellow>.JsonSaveFile(kcm_ProfilePath + m_ConfigFolderName + "/Yellow/" + m_ConfigName + "Yellow" + m_ConfigExtension, m_KeyCard_Yellow_ConfigData);
	}
}